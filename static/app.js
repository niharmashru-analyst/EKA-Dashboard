...
function stores(){
  ...
  if(rankChartRows.length)chart('storePerfRankChart',{...base,grid:{left:55,right:60,top:20,bottom:20,containLabel:true},xAxis:{type:'value',axisLabel:{formatter:v=>pct1(v)}},yAxis:{type:'category',data:rankChartRows.map(x=>x['Store Name']),axisLabel:{fontSize:9}},series:[{name:'Movement %',type:'bar',data:rankChartRows.map(x=>({value:+x['Movement %'].toFixed(1),itemStyle:{color:x['Movement %']>=0?'#0F766E':'#DC2626'}})),label:{show:true,position:'right',formatter:q=>pct1(q.value)}}]});
