...
const metricPairs={
  'Stock':['Stock','Total MRP Value'],'L3M Avg Qty':['L3M Avg Qty','L3M Avg Value'],'LY Qty':['LY Qty','LY Value'],'Current Month Qty':['Current Month Qty','Current Month Value'],
  'Opening Stock Qty':['Opening Stock Qty','Opening Stock Value'],'Inward Qty':['Inward Qty','Inward Value'],'Tertiary Qty':['Tertiary Qty','Tertiary Value'],
  'Closing Stock Qty':['Closing Stock Qty','Closing Stock Value'],'Calculated Closing Qty':['Calculated Closing Qty','Calculated Closing Value'],'Difference Qty':['Difference Qty','Difference Value'],'Actual Closing Qty':['Actual Closing Qty','Actual Closing Value']
};
...
function aggregateBy(rows,keyCol,countCols=[]){
  ...
  const numericCols=sourceCols.filter(c=>c!==keyCol && isNumericColumn(rows,c) && !['MRP','Unit MRP'].includes(String(c).trim()));
  rows.forEach(r=>{
    const raw=r[keyCol];
...
function tableCols(rows){
  const desiredValue=new Set(Object.values(metricPairs).map(x=>x[1]));
  const desiredQty=new Set(Object.values(metricPairs).map(x=>x[0]));
  const source=DATA_COLUMNS.filter(c=>rows.some(r=>Object.prototype.hasOwnProperty.call(r,c))).filter(c=>{
    if(viewMode==='value' && desiredQty.has(c)) return false;
    if(viewMode==='qty' && desiredValue.has(c)) return false;
    return true;
  });
  const extras=[];
...
function comboSkuOption(id,arr,title){if(!arr.length){document.getElementById(id).innerHTML='<div class="empty">No SKUs available.</div>';return}let sf=valueField('Stock'),lf=valueField('L3M Avg Qty'),cf=valueField('Current Month Qty'),yf=valueField('LY Qty');let a=arr.slice().sort((x,y)=>(+y[sf]||0)-(+x[sf]||0)).slice(0,5);chart(id,{...base,animation:false,grid:{left:55,right:25,top:55,bottom:90,containLabel:true},tooltip:{trigger:'axis',axisPointer:{type:'shadow'},formatter:ps=>{let i=ps[0]?.dataIndex??0,x=a[i]||{};return `<b>${esc(x['Product Name'])}</b><br>${metricLabel('Stock')}: ${metricFmt(x[sf])}<br>${metricLabel('L3M Avg Qty')}: ${metricFmt(x[lf])}<br>${metricLabel('Current Month Qty')}: ${metricFmt(x[cf])}<br>${metricLabel('LY Qty')}: ${metricFmt(x[yf])}`}},legend:{show:true,top:5},xAxis:{type:'category',data:a.map(x=>x['Product Name']),axisLabel:{rotate:25,fontSize:9,interval:0,formatter:v=>String(v).length>20?String(v).slice(0,20)+'…':v}},yAxis:{type:'value',axisLabel:{formatter:v=>chartFormat(v,viewMode)}},series:[{name:metricLabel('Stock'),type:'bar',data:a.map(x=>+x[sf]||0),barMaxWidth:32,label:{show:true,position:'top',formatter:q=>metricFmt(q.value)}},{name:metricLabel('L3M Avg Qty'),type:'line',data:a.map(x=>+x[lf]||0),symbol:'circle',symbolSize:7},{name:metricLabel('Current Month Qty'),type:'line',data:a.map(x=>+x[cf]||0),symbol:'circle',symbolSize:7},{name:metricLabel('LY Qty'),type:'line',data:a.map(x=>+x[yf]||0),symbol:'circle',symbolSize:7}]},()=>openChartModal(title,arr))}
function openChartModal(title,arr){
  let ov=document.getElementById('modalOverlay');
  let sf=valueField('Stock'),lf=valueField('L3M Avg Qty'),cf=valueField('Current Month Qty'),yf=valueField('LY Qty');
  document.getElementById('modalTitle').textContent=title+' — Full View';
  document.getElementById('modalSub').textContent=`${arr.length} SKUs • ${viewMode==='value'?'value':'quantity'} view`;
  document.getElementById('modalBody').innerHTML=`<div class="modal-tools"><span class="muted">All SKUs shown below • ${viewMode==='value'?'Value':'Quantity'} view</span></div><div id="modalChart" class="modal-chart"></div>`;
  ov.classList.add('open');
  requestAnimationFrame(()=>{let el=document.getElementById('modalChart');if(!el)return;let c=echarts.init(el);c.setOption({...base,animation:false,grid:{left:75,right:55,top:85,bottom:125,containLabel:true},tooltip:{trigger:'axis',axisPointer:{type:'shadow'},formatter:ps=>{let i=ps[0]?.dataIndex??0,x=arr[i]||{};return `<b>${esc(x['Product Name'])}</b><br>${metricLabel('Stock')}: ${metricFmt(x[sf])}<br>${metricLabel('L3M Avg Qty')}: ${metricFmt(x[lf])}<br>${metricLabel('Current Month Qty')}: ${metricFmt(x[cf])}<br>${metricLabel('LY Qty')}: ${metricFmt(x[yf])}`}},legend:{show:true,top:8},dataZoom:[{type:'inside'},{type:'slider',bottom:18,height:18,start:0,end:100}],xAxis:{type:'category',data:arr.map(x=>x['Product Name']),axisLabel:{rotate:35,interval:0,fontSize:10,formatter:v=>String(v).length>26?String(v).slice(0,26)+'…':v}},yAxis:{type:'value',axisLabel:{formatter:qty}},series:[{name:'Stock Qty',type:'bar',data:arr.map(x=>+x.Stock||0),barMaxWidth:34},{name:'L3M Avg Qty',type:'line',data:arr.map(x=>+x['L3M Avg Qty']||0),symbol:'circle',symbolSize:7},{name:'CM Qty',type:'line',data:arr.map(x=>+x['Current Month Qty']||0),symbol:'circle',symbolSize:7},{name:'LY Qty',type:'line',data:arr.map(x=>+x['LY Qty']||0),symbol:'circle',symbolSize:7}]});c.resize();charts.push(c)});
}
function products(){
...
function stores(){
  const sf=valueField('Stock'),lf=valueField('L3M Avg Qty'),cf=valueField('Current Month Qty'),yf=valueField('LY Qty');
  const rows=aggregateStore(FILTERED);
  const skuRows=aggregateSku(FILTERED);
  const nc=nodCounts(skuRows);
  const hc=healthCounts(rows);
  const top=rows.slice().sort((a,b)=>(+b[sf]||0)-(+a[sf]||0)).slice(0,15);
  const mom=rows.slice().sort((a,b)=>Math.abs((+b[cf]||0)-(+b[yf]||0))-Math.abs((+a[cf]||0)-(+a[yf]||0))).slice(0,12);
  ...
  let cols=tableCols(rows).filter(c=>c!=='MRP'&&c!=='Unit MRP');
  cols=['Rank',...cols.filter(c=>c!=='Rank')];
  document.getElementById('app').innerHTML=`<div class="kpis kpis-4">${kpi('Total Stores',qty(rows.length),'One row per unique outlet')}${kpi(metricLabel('Stock'),metricFmt(sum(rows,valueField('Stock'))),'Current stock')}${kpi(metricLabel('L3M Avg Qty'),metricFmt(sum(rows,valueField('L3M Avg Qty'))),'Run rate')}</div><div class="kpis kpis-4">${kpi(metricLabel('Current Month Qty'),metricFmt(sum(rows,valueField('Current Month Qty'))),'Current month')}${kpi(metricLabel('LY Qty'),metricFmt(sum(rows,valueField('LY Qty'))),'Last year')}${kpi('NOD',qty(totalNod(rows))+' Days','Overall stock cover')}${kpi('CM vs LY',(growthPct(sum(rows,'Current Month Qty'),sum(rows,'LY Qty'))==null?'Not Active LY':pct1(growthPct(sum(rows,'Current Month Qty'),sum(rows,'LY Qty')))),'Growth')}</div><div class="kpis kpis-5">${kpi('NOD <15',qty(nc.lt15),'SKU count','bad')}${kpi('NOD 15–30',qty(nc.n15_30),'SKU count')}${kpi('NOD 31–60',qty(nc.n31_60),'SKU count')}${kpi('NOD >60',qty(nc.gt60),'SKU count','bad')}${kpi('Dead Stock Outlets',qty(hc.dead),'Stock but no L3M movement','bad')}</div><div class="grid2"><div class="card"><div class="card-title">Store Stock vs Sales Run Rate — Top 15</div><div id="storePerfChart" class="chart chart-tall"></div></div><div class="card"><div class="card-title">Store Sales Momentum — Top 12</div><div id="storeMomentumChart" class="chart chart-tall"></div></div></div><div class="grid2"><div class="card"><div class="card-title">Store NOD Distribution</div><div id="storeNodChart" class="chart"></div></div><div class="card"><div class="card-title">Type-wise Store Performance</div><div id="storeTypeChart" class="chart"></div></div></div><div class="card"><div class="card-title">Best &amp; Worst Performing Stores <span class="muted">By Growth % (Current Month vs LY)</span> <span class="seg"><button id="spTop" class="${storePerfView==='top'?'active':''}">Top Performing</button><button id="spWorst" class="${storePerfView==='worst'?'active':''}">Worst Performing</button></span></div><div id="storePerfRankChart" class="chart chart-tall"></div><div class="rank-detail-wrap"><div class="card-title rank-detail-title">${storePerfView==='worst'?'Worst 10':'Top 10'} Store Sales Details <span class="muted">Quantity • CM, LY, L3M Avg & Stock</span></div><div class="table-wrap rank-detail-table"><table><thead><tr><th>Rank</th><th>Store Name</th><th>Growth %</th><th>CM Qty</th><th>LY Qty</th><th>L3M Avg Qty</th><th>Stock Qty</th></tr></thead><tbody id="storeRankDetails"></tbody></table></div></div></div><div class="card"><div class="card-title">Store Analysis <span class="muted">${rows.length.toLocaleString('en-IN')} unique outlets • click a row to see every SKU in that outlet</span></div>${table(rows,cols,'stores',Infinity,'Store Name')}</div>`;
  const topR=top.slice().reverse();
  chart('storePerfChart',{...base,legend:{show:true,top:5},dataZoom:[{type:'inside'},{type:'slider',bottom:8,height:16}],xAxis:{type:'value',axisLabel:{formatter:v=>chartFormat(v,viewMode)}},yAxis:{type:'category',data:topR.map(x=>x['Store Name']),axisLabel:{fontSize:9}},series:[{name:metricLabel('Stock'),type:'bar',data:topR.map(x=>+x[sf]||0),label:{show:true,position:'right',formatter:q=>metricFmt(q.value)}},{name:metricLabel('L3M Avg Qty'),type:'bar',data:topR.map(x=>+x[lf]||0),label:{show:true,position:'right',formatter:q=>metricFmt(q.value)}},{name:metricLabel('Current Month Qty'),type:'bar',data:topR.map(x=>+x[cf]||0),label:{show:true,position:'right',formatter:q=>metricFmt(q.value)}}]});
  chart('storeMomentumChart',{...base,legend:{show:true,top:5},dataZoom:[{type:'inside'},{type:'slider',bottom:8,height:16}],xAxis:{type:'category',data:mom.map(x=>x['Store Name']),axisLabel:{rotate:28,fontSize:9,interval:0,formatter:v=>String(v).length>18?String(v).slice(0,18)+'…':v}},yAxis:{type:'value',axisLabel:{formatter:v=>chartFormat(v,viewMode)}},series:[{name:metricLabel('LY Qty'),type:'line',data:mom.map(x=>+x[yf]||0),symbol:'circle',symbolSize:7,label:{show:true,position:'top',formatter:q=>metricFmt(q.value)}},{name:metricLabel('L3M Avg Qty'),type:'line',data:mom.map(x=>+x[lf]||0),symbol:'circle',symbolSize:7,label:{show:true,position:'top',formatter:q=>metricFmt(q.value)}},{name:metricLabel('Current Month Qty'),type:'line',data:mom.map(x=>+x[cf]||0),symbol:'circle',symbolSize:7,label:{show:true,position:'top',formatter:q=>metricFmt(q.value)}}]});
  ...
  const tm={}; FILTERED.forEach(r=>{const t=canonicalType(r.Type);tm[t]??={stock:0,l3m:0,cm:0,ly:0};tm[t].stock+=+r[sf]||0;tm[t].l3m+=+r[lf]||0;tm[t].cm+=+r[cf]||0;tm[t].ly+=+r[yf]||0}); types.forEach(t=>tm[t]??={stock:0,l3m:0,cm:0,ly:0});
  chart('storeTypeChart',{...base,legend:{show:true,top:5},xAxis:{type:'category',data:types},yAxis:{type:'value',axisLabel:{formatter:v=>chartFormat(v,viewMode)}},series:[{name:metricLabel('Stock'),type:'bar',data:types.map(t=>tm[t].stock),label:{show:true,position:'top',formatter:q=>metricFmt(q.value)}},{name:metricLabel('L3M Avg Qty'),type:'line',data:types.map(t=>tm[t].l3m),symbol:'circle',symbolSize:7,label:{show:true,position:'top',formatter:q=>metricFmt(q.value)}},{name:metricLabel('Current Month Qty'),type:'line',data:types.map(t=>tm[t].cm),symbol:'circle',symbolSize:7,label:{show:true,position:'top',formatter:q=>metricFmt(q.value)}},{name:metricLabel('LY Qty'),type:'line',data:types.map(t=>tm[t].ly),symbol:'circle',symbolSize:7,label:{show:true,position:'top',formatter:q=>metricFmt(q.value)}}]});
...
function variance(){
  let rows=VAR.slice();
  if(varianceShop) rows=rows.filter(r=>String(r['Store Name']||'')===varianceShop);
  if(varianceSkuSearch){const q=varianceSkuSearch.toLowerCase();rows=rows.filter(r=>[r['EAN Code'],r['Product Name'],r['Store Name']].some(v=>String(v||'').toLowerCase().includes(q)))}
  const physicalIssues=rows.filter(r=>r['Live Submission']&&Math.abs(+r['Difference Qty']||0)>0);
  const stockIssues=rows.filter(r=>Math.abs(+r['Stock Variance Qty']||0)>0);
  if(varianceIssueOnly) rows=rows.filter(r=>Math.abs(+r['Stock Variance Qty']||0)>0 || (r['Live Submission']&&Math.abs(+r['Difference Qty']||0)>0));
  const absPhysical=rows.reduce((a,r)=>a+Math.abs(+r['Difference Qty']||0),0);
  const stockVariance=rows.reduce((a,r)=>a+(Number(r['Stock Variance Qty'])||0),0);
  const shops=[...new Set(VAR.map(r=>r['Store Name']).filter(Boolean))].sort();
  const live=rows.filter(r=>r['Live Submission']),liveQty=sum(live,'Actual Closing Qty');
  document.getElementById('app').innerHTML=`<div class="variance-note"><span>BETA</span> Variance Analysis — Quantity basis only.</div><div class="variance-filters"><span class="seg"><button class="active">Qty</button></span><select id="vShop"><option value="">All Shops</option>${shops.map(s=>`<option ${s===varianceShop?'selected':''} value="${esc(s)}">${esc(s)}</option>`).join('')}</select><input id="vSku" placeholder="Search SKU / EAN / Product" value="${esc(varianceSkuSearch)}"><label class="check"><input id="issueOnly" type="checkbox" ${varianceIssueOnly?'checked':''}> Issues only</label><button id="vCsv" class="btn secondary small">Download CSV</button></div><div class="kpis kpis-5">${kpi('Opening Stock',qty(sum(rows,'Opening Stock Qty')),'Opening Qty')}${kpi('Inward Receipts',qty(sum(rows,'Inward Qty')),'Inward Qty')}${kpi('Tertiary',qty(sum(rows,'Tertiary Qty')),'Dispatch Qty')}${kpi('System Stock',qty(sum(rows,'System Stock Qty')),'Current system Qty')}${kpi('Physical Stock',qty(liveQty),'Latest submitted Qty')}</div><div class="kpis kpis-6">${kpi('SKU Count',qty(new Set(rows.map(r=>String(r['EAN Code']??r['Product Name']??'').trim()).filter(Boolean))),'Unique SKUs')}${kpi('Error SKUs',qty(new Set(rows.filter(r=>Math.abs(+r['Stock Variance Qty']||0)>0 || (r['Live Submission']&&Math.abs(+r['Difference Qty']||0)>0)).map(r=>String(r['EAN Code']??r['Product Name']??'').trim()).filter(Boolean)).size),'SKUs with variance',((stockIssues.length+physicalIssues.length)?'bad':'good'))}${kpi('Store Count',qty(new Set(rows.map(r=>String(r['Store Name']||'').trim()).filter(Boolean)).size),'Unique stores')}${kpi('Error Stores',qty(new Set(rows.filter(r=>Math.abs(+r['Stock Variance Qty']||0)>0 || (r['Live Submission']&&Math.abs(+r['Difference Qty']||0)>0)).map(r=>String(r['Store Name']||'').trim()).filter(Boolean)).size),'Stores with variance',((stockIssues.length+physicalIssues.length)?'bad':'good'))}${kpi('Stock Variance',qty(stockVariance),'Calculated Closing vs Closing Stock',stockIssues.length?'bad':'good')}${kpi('Physical Variance',qty(absPhysical),'Physical vs System Stock',physicalIssues.length?'bad':'good')}</div><div class="grid2"><div class="card"><div class="card-title">Stock Variance by Shop — Qty</div><div id="stockVarianceShopChart" class="chart chart-tall"></div></div><div class="card"><div class="card-title">Physical Variance by Shop — Qty</div><div id="varianceShopChart" class="chart chart-tall"></div></div></div><div class="grid2"><div class="card"><div class="card-title">Top Variance SKUs — Qty</div><div id="varianceSkuChart" class="chart chart-tall"></div></div><div class="card"><div class="card-title">Variance Direction — Qty</div><div id="varianceDirectionChart" class="chart"></div></div></div><div class="card"><div class="card-title">Variance Detail — Quantity</div>${table(rows,varianceCols(rows),'variance',250)}</div>`;
  document.getElementById('vShop').onchange=e=>{varianceShop=e.target.value;variance()};
  document.getElementById('vSku').oninput=e=>{varianceSkuSearch=e.target.value;variance()};
...
function openAI(){document.getElementById('aiPanel')?.classList.add('open');document.getElementById('aiBackdrop')?.classList.add('open');document.getElementById('aiFab')?.classList.add('hidden');document.getElementById('aiPanel')?.setAttribute('aria-hidden','false');setTimeout(()=>document.getElementById('aiQuestion')?.focus(),50)}
function closeAI(){document.getElementById('aiPanel')?.classList.remove('open');document.getElementById('aiBackdrop')?.classList.remove('open');document.getElementById('aiFab')?.classList.remove('hidden');document.getElementById('aiPanel')?.setAttribute('aria-hidden','true')}
