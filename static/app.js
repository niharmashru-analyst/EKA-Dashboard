...
function aiAppend(role,text){const box=document.getElementById('aiMessages');if(!box)return;const d=document.createElement('div');d.className='ai-msg '+role;d.innerHTML=`<b>${role==='user'?'You':'E.K.A. AI'}</b><div>${esc(text)}</div>`;box.appendChild(d);box.scrollTop=box.scrollHeight;return d}
