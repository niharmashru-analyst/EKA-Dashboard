...
function tableSummary(rows,id){
  const rr=rows||[];
  const skuSet=new Set(rr.map(r=>String(r['EAN Code']??r['Product Name']??'').trim()).filter(Boolean));
  const storeSet=new Set(rr.map(r=>String(r['Store Name']??'').trim()).filter(Boolean));
  if(id==='variance'){
    const errorRows=rr.filter(r=>Math.abs(Number(r['Stock Variance Qty']||0))>0 || (r['Live Submission']&&Math.abs(Number(r['Difference Qty']||0))>0));
    const errorSku=new Set(errorRows.map(r=>String(r['EAN Code']??r['Product Name']??'').trim()).filter(Boolean));
    const errorStore=new Set(errorRows.map(r=>String(r['Store Name']??'').trim()).filter(Boolean));
    return `${skuSet.size.toLocaleString('en-IN')} SKUs • ${errorSku.size.toLocaleString('en-IN')} Error SKUs • ${storeSet.size.toLocaleString('en-IN')} Stores • ${errorStore.size.toLocaleString('en-IN')} Error Stores`;
  }
  if(id==='products') return `${skuSet.size.toLocaleString('en-IN')} SKUs • ${storeSet.size.toLocaleString('en-IN')} Stores`;
  if(id==='stores') return `${storeSet.size.toLocaleString('en-IN')} Stores • ${skuSet.size.toLocaleString('en-IN')} SKUs`;
  return `${skuSet.size.toLocaleString('en-IN')} SKUs • ${storeSet.size.toLocaleString('en-IN')} Stores`;
}
...
function varianceCols(rows){
  const preferred=['Store Name','EAN Code','Product Name','Opening Stock Qty','Inward Qty','Tertiary Qty','Calculated Closing Qty','Closing Stock Qty','Stock Variance Qty','System Stock Qty','Actual Closing Qty','Difference Qty','Live Submission'];
...
function variance(){
  ...
  const absStock=rows.reduce((a,r)=>a+Math.abs(+r['Stock Variance Qty']||0),0);
